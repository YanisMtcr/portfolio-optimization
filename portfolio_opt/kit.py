...
def drawdown(r):
    """
    Calcule le drawdown pour une Series OU pour chaque colonne d'un DataFrame.
    Retourne un DataFrame multi-indexé (Indicator, Portfolio) en colonnes.
    """
    import pandas as pd
    ...
    if isinstance(r, pd.DataFrame):
        ...
        for col in r.columns:
            df = drawdown(r[col])
            for indicator in ["Wealth", "Previous Peak", "Drawdown"]:
                s = df[indicator]
                s.name = (indicator, col)
                frames.append(s)
        result = pd.concat(frames, axis=1)
        result.columns = pd.MultiIndex.from_tuples(result.columns, names=["Indicator", "Portfolio"])
        return result
        ...
        raise TypeError("r doit être une Series ou un DataFrame")
...
def msr(riskfree_rate, er, cov, min_weight_factor=None):
    """
    Maximum Sharpe Ratio optimizer with optional lower bound constraints.
    
    min_weight_factor: if set, imposes min weight = min_weight_factor * 1/N
                       e.g., 0.5 means min = 1/(2N)
    """
    n = er.shape[0]
    init_guess = np.repeat(1/n, n)

    if min_weight_factor is not None:
        min_weight = min_weight_factor / n
        if n * min_weight > 1:
            raise ValueError(f"Infeasible constraint: total min weights = {n * min_weight} > 1")
...
def gmv(cov,min_weight_factor=None):
    """
    ...
    n = cov.shape[0]
    if min_weight_factor is None:
        return msr(0, np.repeat(1, n), cov)
    else:
        return msr(0, np.repeat(1, n), cov, min_weight_factor=min_weight_factor)
...
def plot_ef(n_points, er, cov, style='.-', legend=False, show_cml=False,
            riskfree_rate=0, show_ew=False, show_gmv=False, ax=None):
    """
    Trace la frontière efficiente pour un ensemble d'actifs.
    Peut également afficher :
        - la Capital Market Line (CML)
        - le portefeuille Equally Weighted (EW)
        - le portefeuille Global Minimum Variance (GMV)
    
    Params :
    - n_points : int, nombre de portefeuilles à générer
    - er : Series, rendements espérés
    - cov : DataFrame, matrice de covariance
    - riskfree_rate : float, taux sans risque pour la CML
    """
    # Vérification / conversion
    er = pd.Series(er)
    cov = pd.DataFrame(cov)
    assert er.shape[0] == cov.shape[0], "Dimensions mismatch entre er et cov"
    ...
    ef.plot(x="Volatility", y="Returns", style=style, ax=ax, label="Efficient Frontier")
    ...
    # Portefeuille Equally Weighted (EW)
    if show_ew:
        n = er.shape[0]
        w_ew = np.repeat(1/n, n)
        r_ew = portfolio_return(w_ew, er)
        vol_ew = portfolio_vol(w_ew, cov)
        ax.plot(vol_ew, r_ew, color='orange', marker='o', markersize=10, label='EW Portfolio')
